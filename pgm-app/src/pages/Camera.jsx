...
export default function Camera() {
  ...
  const startCamera = async () => {
    try {
      ...
      // Process any offers that arrived before the camera was ready
      for (const [peerId, sdp] of pendingOffers.current.entries()) {
  ...
  const handleOffer = async (sdp, peerId) => {
    ...
    pc.onconnectionstatechange = () => {
      console.log(`[PGM Camera] ${peerId} connection: ${pc.connectionState}`);
    };

    await pc.setRemoteDescription(new RTCSessionDescription(sdp));
    const answer = await pc.createAnswer();
    await pc.setLocalDescription(answer);
    wsRef.current?.send(JSON.stringify({ type: "sdp-answer", sdp: answer, peerId }));
  };
  ...
  return (
    <div style={{
          ...
          <div style={{ width: "100%", height: "100%", background: "#0d1117", display: "flex", alignItems: "center", justifyContent: "center", flexDirection: "column", gap: 16 }}>
            {camError ? (
              <>
                ...
                <button onClick={startCamera} style={{ background: "#1f2937", border: "1px solid #374151", borderRadius: "8px", color: "#d1d5db", fontSize: 13, fontWeight: 600, padding: "10px 24px", cursor: "pointer", fontFamily: "inherit" }}>Try again</button>
              </>
            ) : (
      ...
      <div style={{
        position: "relative", zIndex: 11, display: "flex", alignItems: "center", justifyContent: "space-between",
        padding: "18px 20px", background: "linear-gradient(to bottom, rgba(0,0,0,0.72), transparent)",
      }}>
        ...
        {s.badge && (
          <div style={{
            background: s.badgeBg, color: s.badgeColor, fontSize: 11, fontWeight: 800,
            letterSpacing: "0.12em", padding: "5px 14px", borderRadius: "6px", textTransform: "uppercase",
      ...
      <div style={{
        ...
        padding: "0 20px 40px", background: "linear-gradient(to top, rgba(0,0,0,0.75), transparent)",
      }}>
        <p style={{ fontSize: 28, fontWeight: 700, color: "#f9fafb", margin: "0 0 4px", letterSpacing: "-0.02em", lineHeight: 1.1 }}>{camName}</p>
