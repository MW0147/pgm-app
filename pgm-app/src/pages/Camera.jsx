...
// These would be set via URL params in production e.g. /camera?id=cam1&name=Cam+1&label=Stage+Left
...
export default function Camera() {
  ...
  const camLabel = getUrlParam("label", "Stage Left");
    ...
    const connect = () => {
      ...
      ws.onmessage = async (event) => {
        ...
        switch (msg.type) {
          ...
          // Director wants to open a WebRTC connection
          case "sdp-offer": {
            const pc = new RTCPeerConnection({ iceServers: ICE_SERVERS });
            pcRef.current = pc;

            // Add our local camera track so the director can see us
          ...
          // Tally state update from director
          case "tally": {
            setTally(msg.state); // "idle" | "preview" | "program"
            break;
          }
        }
      };
